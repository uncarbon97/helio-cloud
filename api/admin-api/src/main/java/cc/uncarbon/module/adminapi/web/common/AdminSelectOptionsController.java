...
public class AdminSelectOptionsController {
    ...
    public ApiResult<List<SelectOptionItemVO>> depts(YesOrNoEnum demo) {
        if (demo == YesOrNoEnum.YES) {
            ...
            List<SelectOptionItemVO> ret = SelectOptionItemVO.listOf(sysDeptFacade.adminList(), SysDeptBO::getId, SysDeptBO::getTitle, SysDeptBO::getParentId);
            for (SelectOptionItemVO item : ret) {
        ...
        // 默认返回
        return ApiResult.data(SelectOptionItemVO.listOf(sysDeptFacade.adminList(), SysDeptBO::getId, SysDeptBO::getTitle));
    }
